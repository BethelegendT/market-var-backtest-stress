...
def ewma_sigma(returns: pd.Series, lam: float = 0.94) -> pd.Series:
    ...
    s2 = np.zeros_like(r)
    s2[0] = np.var(r[:30]) if len(r) >= 30 else np.var(r)
    for t in range(1, len(r)):
        s2[t] = lam*s2[t-1] + (1-lam)*(r[t-1]**2)
    return pd.Series(np.sqrt(s2), index=_clean(returns).index)
...
def rolling_var(returns: pd.Series, model: str, window: int = 500, alpha: float = 0.99) -> pd.Series:
    r = _clean(returns)
    vals, idx = [], []
